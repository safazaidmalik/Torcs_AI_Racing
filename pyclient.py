...
verbose = False
using_steering_wheel = False
automatic = True
interface = ManualMovement.Interface()
expert = ManualMovement.Expert(interface, automatic=automatic)
d = driver.Driver(arguments.stage, interface, expert)
...
while not shutdownClient:
    ...
    while True:
        ...
        if buf is not None and buf.decode().find('***shutdown***') >= 0:
            d.onShutDown()
            shutdownClient = True
            print('Client Shutdown')
            break

        if buf is not None and buf.decode().find('***restart***') >= 0:
            d.onRestart()
